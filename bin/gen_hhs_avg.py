...
class DataFile(pd.DataFrame):
    ...
    def _pad_with_zeros(self, ydat, factor=8):
        ...
        pre = (numzeros+1)//2
        post = numzeros//2
        return np.concatenate([np.zeros(pre), np.transpose(ydat), np.zeros(post)])

# --------------------------------------------------------------------------------

def read_command_line():
    parser = AP()
    parser.add_argument('-m','--minimum_intensity', type=float, help='option to change minimum intensity to use in average, given as percentage of peak intensity, rather than use all intensities in file')
    parser.add_argument('-di','--delta_intensity', action='store_true', help='option to calculate several averages using various different values of delta I')
    parser.add_argument('-w','--omega_0', action='store_true', help='option to calculate several averages using various different beam radii')
...
df=DataFile("expec_v.csv")
peak_intensity, intensities = get_intensities(df)
dI=intensities[0]-intensities[1]

if args["minimum_intensity"]:
    I_min = peak_intensity*args["minimum_intensity"] #Get intensity corresponding to percentage of peak
    I_trunc = intensities[np.abs(intensities-I_min).argmin()] #Get intensity column corresponding to I_min at which to truncate
    df =df.truncate(before=str(I_trunc), after=None, axis=1) #truncate dataframe

vph = df.HHG()

#Write peak intensity spectra to file
single = np.real(np.abs(vph.iloc[:,1])**2) #assumes peak intensity is first in csv file - generalise?
...
for area in areas:
    ...
    fc_avg = np.real(np.abs((1j*vph["Freq"].values/(2*np.pi*25*10**7))*averaged_data_full)**2) #coherent phase weighted average
    ...
    n_coh.to_csv("naive_coherent_area_"+str(area)+"_step_"+str(round(dI,3))+".csv", index = False)

    #Write full coherent average to file
    f_coh = pd.DataFrame({"Freq" : vph["Freq"].values*27.212, "0001_z" : fc_avg})
    f_coh.to_csv("full_coherent_area_"+str(area)+"_step_"+str(round(dI,3))+".csv", index = False)
